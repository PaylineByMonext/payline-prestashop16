...
<h1 class="page-heading">{l s='My subscribe Payline' mod='payline'}</h1>
...
<div class="block-center" id="block-history">
	...
	<table id="order-list" class="table table-bordered footab default footable-loaded footable">
		<thead>
			<tr>
				<th class="first_item footable-first-column" data-hide="phone,tablet" data-sort-ignore="true">{l s='N°subscribe' mod='payline'}</th>
				<th class="item footable-sortable">{l s='Start Date' mod='payline'}</th>
				<th class="item footable-sortable">{l s='Next deadline' mod='payline'}</th>
				<th class="item">{l s='Next amount' mod='payline'}</th>
				<th class="item footable-sortable">{l s='Status' mod='payline'}</th>
				<th class="last_item footable-last-column" data-hide="phone,tablet" data-sort-ignore="true"> </th>
			</tr>
		</thead>
		<tbody>
		{foreach from=$subscribes item=subscribe name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				...
				<td class="history_method">
					...
					<a class="color-myaccount suspended" id="{$subscribe.paymentRecordId}|{if isset($subscribe.nextDate)}{$subscribe.nextDate}|{/if}{if isset($subscribe.2nextDate)}{$subscribe.2nextDate}|{/if}{$paylineNumberPending}|{$subscribe.pendingNumber}" href="#suspendSubscribe">{l s='suspend' mod='payline'}</a> | 
						...
						{foreach from=$subscribe.orders item=order name=myOrder}
							<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
								<td class="history_link bold">
									...
									<a class="color-myaccount" href="{$link->getPageLink('history', true)|escape:'html':'UTF-8'}">{l s='#'}{$order.id_order|string_format:"%06d"}</a>
								</td>
								...
								<td class="history_detail">
									<a class="color-myaccount" href="{$link->getPageLink('history', true)|escape:'html':'UTF-8'}">{l s='details' mod='payline'}</a>
								</td>
							</tr>
						{/foreach}
						</tbody>
...
<div style="display:none">
	...
	<form action="{$link->getModuleLink('payline', 'subscription', array(), true)}" method="post">
